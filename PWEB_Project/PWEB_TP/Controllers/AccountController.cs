        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            using (var context = new ApplicationDbContext())
            {
                if (ModelState.IsValid)
                {
                    ...
                    var resultRoles = UserManager.AddToRole(user.Id, model.Role);
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        {
                            var client = new Clients { Name = model.Name, NIF = model.NIF, IdentityId = user.Id, Status = "Accepted" };
        ...
        public async Task<ActionResult> RegisterEmployee(RegisterEmployeeViewModel model)
        {
            using (var context = new ApplicationDbContext())
            {
                if (ModelState.IsValid)
                {
                    var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                    var result = await UserManager.CreateAsync(user, model.Password);
                    var resultRoles = UserManager.AddToRole(user.Id, "Employee");
                    ...
                    if (result.Succeeded)
                    {
                        try
                        {
                            ...
                            { 
                                ...
                                IdentityId = user.Id, 
                                Company = GetCompany(), 
                                Company_Id = GetCompany().IdCompany,
                                Status = "Accepted"
                            };
